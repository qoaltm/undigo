  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        {particles.map((p, i) => (
          <div key={i} className="absolute rounded-full" style={{ ...p, animation: `ppBlink ${p.animationDuration} ${p.animationDelay} linear infinite` }} />
        ))}
      </div>
      ...
      <div
        ...
        style={{ inset: '-50%', background: `linear-gradient(135deg, transparent 20%, ${theme.accentSoft} 50%, transparent 80%)`, animation: 'ppSweep 8s ease-in-out infinite alternate' }}
      />
      ...
      <div className="relative flex flex-col items-center gap-3 z-[1]">
        ...
        {[300, 220, 155].map((size, i) => (
          <div key={i} className="absolute rounded-full border top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
            style={{ width: size, height: size, borderColor: `${theme.accent}${['18','28','40'][i]}`, animation: `ppPulse 4s ease-in-out ${i * 0.5}s infinite` }} />
        ))}
        ...
        <div className="flex items-center gap-2.5 relative z-[2]">
          {/* Side lines */}
          <div className="absolute w-px h-20 left-[-40px] top-1/2 -translate-y-1/2" style={{ background: `linear-gradient(180deg, transparent, ${theme.accent}50, transparent)` }} />
          <div className="absolute w-px h-20 right-[-40px] top-1/2 -translate-y-1/2" style={{ background: `linear-gradient(180deg, transparent, ${theme.accent}50, transparent)` }} />
      ...
      <p className="absolute bottom-10 text-[0.6rem] tracking-[0.35em] uppercase z-[1]" style={{ color: `${theme.particleColor}70` }}>
        Pratinjau Undangan
      </p>
...
export default function PreviewPage({ onCheckout }) {
  ...
  const { component: TemplateComponent, name, price, id, initials, theme } = entry;
  const fmtPrice = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(price);
    ...
    <div className="min-h-screen flex flex-col bg-[#060c14] font-body">
      ...
      >
        ...
        <button
          ...
          onClick={() => onCheckout?.({ id, name, price })}
        >
          Pilih Template
